...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                             QLineEdit, QPushButton, QProgressBar, QMessageBox, QFrame, QComboBox, QListView) # <--- 加上 QListView
...
class Stage4TransitArriveApp(QDialog):
    def __init__(self, parent, selected_files_db):
        ...
        # 初始化安全通讯器
        self.updater = TransitArriveUpdater()
    ...
    def setup_ui(self):
        ...
        # 🌟 强力修复：显式创建视图，并强制开启垂直滚动条
        view = QListView()
        view.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        self.tz_combo.setView(view)

        # 🌟 关键：限制高度为 10，超出必须滚动
        self.tz_combo.setMaxVisibleItems(10)
        ...
        self.gen_btn.setFixedSize(180, 42)  # 🌟 强行撑开按钮防止文字折叠
    ...
    def auto_format(self, widget, text, mode):
        if mode == 'EN_SPACE':
            ...
            # 国家：纯中文简体 (拦截所有英文字母和数字)
            cleaned = re.sub(r'[^\u4e00-\u9fa5]', '', text)
        elif mode == 'EN_ONLY':
            # 机场三字码：纯英文，无空格 (大写)，最多限制3位
    ...
    def _giant_table_worker(self, file_data, timezone, location, country, airport, flight):
        ...
        try:
            ...
            output_folder = os.path.join(source_dir, "-4-到达中转机场-")
            os.makedirs(output_folder, exist_ok=True)

            output_filepath = os.path.join(output_folder, f"{bl_no} 到达中转机场--{pkg_count}.xlsx")
            ...
            if HAS_XLSXWRITER:
                ...
                for row_num, pkg in enumerate(unique_pkgs, 1):
                    ...
                    worksheet.write_string(row_num, 3, "到达中转机场")
                    worksheet.write_string(row_num, 4, "Arrived at the Transit Airport")
                    ...
                    worksheet.write_string(row_num, 12, "中转机场")
    ...
    def _open_output_folder(self):
        try:
            ...
            target_dir = os.path.join(os.path.dirname(first_file["file_path"]), "-4-到达中转机场-")
