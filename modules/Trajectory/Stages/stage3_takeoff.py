...
class Stage3TakeoffApp(QDialog):
    ...
    def setup_ui(self):
        ...
        self.entry_flight.setPlaceholderText("例: CX888")
        self.entry_flight.setStyleSheet("""
            ...
            QLineEdit:focus { border: 2px solid #17a2b8; }
        """)
    ...
    def _giant_table_worker(self, file_data, flight):
        ...
        try:
            ...
            # 🌟 文件夹：-3-已起飞-
            output_folder = os.path.join(source_dir, "-3-已起飞-")
            os.makedirs(output_folder, exist_ok=True)

            output_filepath = os.path.join(output_folder, f"{bl_no} 已起飞--{pkg_count}.xlsx")
            ...
            if HAS_XLSXWRITER:
                ...
                for row_num, pkg in enumerate(unique_pkgs, 1):
                    ...
                    worksheet.write_string(row_num, 3, "已起飞")
                    worksheet.write_string(row_num, 4, "Departed from the Original Airport")
    ...
    def _open_output_folder(self):
        try:
            ...
            target_dir = os.path.join(os.path.dirname(first_file["file_path"]), "-3-已起飞-")
