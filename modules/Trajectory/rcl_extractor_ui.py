...
def parse_single_pdf(file_path):
    ...
    try:
        with pdfplumber.open(file_path) as pdf:
            if not pdf.pages: return {awb_no: "文件损坏"}
            full_text = "\n".join([page.extract_text() or "" for page in pdf.pages[:2]])
            if not full_text.strip(): return {awb_no: "图片PDF/需人工"}
            results = {}
            ...
            if matches_hactl:
                ...
                return {awb_no: datetime_str}
            return {awb_no: "未提取到时间"}
    except Exception:
        return {awb_no: "解析出错"}
...
class RclExtractorApp(QDialog):
    ...
    def setup_ui(self):
        ...
        left_lyt.addWidget(self.lbl_drop)
        ...
        # 2. 中层工具栏 (🌟 绝杀优化：按钮大升级！三大控制键完美等宽 130px 对齐，立体浮雕感拉满)
        toolbar = QHBoxLayout()
        ...
        for btn in [self.btn_import, self.btn_clear, self.btn_process]:
            ...
            btn.setFixedWidth(130)  # 🌟 强力调教：全员等宽 130 像素，排版极其整严对称
            btn.setFixedHeight(38)  # 舒适饱满大高度

        toolbar.addWidget(self.btn_import)
        toolbar.addWidget(self.btn_clear)
        toolbar.addWidget(self.btn_process)  # 🌟 顺次紧凑排布
        toolbar.addStretch()
        ...
        self.table.setHorizontalHeaderLabels(["提单号", "接收时间"])
    ...
    def handle_drop_files(self, files):
        if files:
            ...
            self.btn_process.setStyleSheet("""
                QPushButton { background-color: #198754; color: white; border: 1px solid #157347; border-radius: 4px; border-bottom: 3px solid #146c43; font-weight: bold; }
                QPushButton:hover { background-color: #157347; }
                QPushButton:pressed { border-bottom: 1px solid #146c43; padding-top: 8px; }
            """)
    ...
    def clear_all(self):
        ...
        self.btn_process.setEnabled(False)
        self.btn_process.setStyleSheet("""
            QPushButton { background-color: #a5b1c2; color: white; border-radius: 4px; border: none; font-weight: bold; }
    ...
    def process_data(self):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            for res_dict in executor.map(parse_single_pdf, self.file_paths):
                if not res_dict: continue
                for awb, time_str in res_dict.items():
                    if awb not in pdf_data:
                        pdf_data[awb] = time_str
                    else:
                        pdf_data[awb] = get_earliest_time(pdf_data[awb], time_str)
        ...
        if orders:
            ...
            for awb in orders:
                self.result_list.append({"提单号": awb, "接收时间": pdf_data.get(awb, "文件未导入")})
            for awb in (parsed_awbs - set(orders)):
                self.result_list.append({"提单号": awb, "接收时间": pdf_data[awb]})
        else:
            for awb, time_str in pdf_data.items():
                self.result_list.append({"提单号": awb, "接收时间": time_str})
    ...
    def copy_table(self):
        ...
        for idx, item in enumerate(self.result_list, 1):
            text += f"{idx}\t{item['提单号']}\t{item['接收时间']}\n"
        QApplication.clipboard().setText(text)
        QMessageBox.information(self, "成功", "已复制，可直接粘贴到 Excel")
    ...
    def export_excel(self):
        ...
        if path:
            ...
            QMessageBox.information(self, "成功", "导出完毕！")
